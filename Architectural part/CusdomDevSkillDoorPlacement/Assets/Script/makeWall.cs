        ...
        if (Input.GetMouseButtonDown(0))
        {
            ...
            if (temp.tag == "floor"|| temp.tag == "wall"|| temp.tag == "pillar"|| temp.tag == "outerWall")
            {
                ...
                else
                {
                    ...
                    //obj.transform.rotation = Quaternion.Euler(0f,(Mathf.Asin(Mathf.Abs(end.x-start.x)/width))*Mathf.Rad2Deg,0f);
                    //obj.transform.Rotate(Vector3.forward, Mathf.Asin((end.x - start.x) / width), Space.Self);
            ...
            Debug.Log("placingWall: "+placingWall);

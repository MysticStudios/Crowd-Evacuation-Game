...
public class bringUpMenu : MonoBehaviour
{
    ...
    IEnumerator getHeatMapData()
    {
        ...
        foreach (XmlNode node in doc.DocumentElement.ChildNodes)
        {
            ...
            {
                if (cnode.Name == "LevelOfService")
                {
                    tlos = cnode.InnerText;
                }
                if (cnode.Name == "LevelOfAggression")
                {
                    tloa = cnode.InnerText;
                }
                if (cnode.Name == "Homogeneity")
                {
                    thomo = cnode.InnerText;
                }
            ...
            if (nodetime < mintime && (tlos.Trim().Equals(los.Trim())) && (tloa.Trim().Equals(loa.Trim())) && (thomo.Trim().Equals(homo.Trim())))
            {
                mintime = nodetime;
                runId = tempRunId;
                wallsXML = node;

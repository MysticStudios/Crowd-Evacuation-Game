...
public class NavigationController : MonoBehaviour {
    ...
    void assignAttributes(string los,string loa,string homog)
    {
        ...
        if(loa.Equals("Low"))
        {
            ...
            {
                navMeshComponent.speed = 5f + 3 * speedValue;//2-5
            }

            //figure out acceleration
            //should be between 6 and 10
            float accelerationValue = UnityEngine.Random.value;

            if(accelerationValue <= 0.7f)
            {
                navMeshComponent.acceleration = 3 + 2 * accelerationValue + 2 * speedValue;//6-10
            }
            else
            {
                navMeshComponent.acceleration = 6 + 2 * accelerationValue + 2 * speedValue;//6-10
            }
